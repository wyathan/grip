...
func CreateTestContextFile(cid string, dig string, dd string, sz uint64) *gripdata.ContextFile {
...
func TestContextFileWrap(t *testing.T) {
	...
	n1 := CreateTestContextFile("context", "1", "dd1", 10)
	n1.Snapshot = true
	...
	n2 := CreateTestContextFile("context", "2", "dd2", 10).PushDep(n1.DataDepDig)
	...
	n3 := CreateTestContextFile("context", "3", "dd3", 100)
	n3.Snapshot = true
	...
	n4 := CreateTestContextFile("context", "4", "dd4", 10).PushDep(n3.DataDepDig)
	...
	n5 := CreateTestContextFile("context", "5", "dd5", 10).PushDep(n2.DataDepDig)
	...
	// 6: 4, 5, 3, 2, 1
	n6 := CreateTestContextFile("context", "6", "dd6", 10).PushDep(n4.DataDepDig).PushDep(n5.DataDepDig)
	n6.Snapshot = true
	...
	// 7: 4, 5, 3, 2, 1
	n7 := CreateTestContextFile("context", "7", "dd7", 10).PushDep(n6.DataDepDig)
	...
	// 8: 4, 3
	n8 := CreateTestContextFile("context", "8", "dd8", 10).PushDep(n5.DataDepDig)
	...
	n9 := CreateTestContextFile("context", "9", "dd9", 10).PushDep(n4.DataDepDig)
	...
	n10 := CreateTestContextFile("context", "10", "dd10", 10).PushDep(n9.DataDepDig)
	...
	n11 := CreateTestContextFile("context", "11", "dd11", 10).PushDep(n8.DataDepDig)
	...
	n12 := CreateTestContextFile("context", "12", "dd12", 10).PushDep(n6.DataDepDig)
	...
	n13 := CreateTestContextFile("context", "13", "dd13", 10).PushDep(n2.DataDepDig)
	...
	n14 := CreateTestContextFile("context", "14", "dd14", 200)
	n14.Snapshot = true
	...
	n15 := CreateTestContextFile("context", "15", "dd15", 10).PushDep(n7.DataDepDig)
	n15.Snapshot = true
	...
	n16 := CreateTestContextFile("context", "16", "dd16", 10).PushDep(n10.DataDepDig)
	w16, err16 := db.StoreContextFile(n16)
	if err16 != nil || !(w16.Depth == 0 && w16.Head && !w16.Leaf && !w16.CoveredBySnapshot) {
		t.Error()
	}

	n17 := CreateTestContextFile("context", "17", "dd17", 10).PushDep(n16.DataDepDig)
	w17, err17 := db.StoreContextFile(n17)
	...
	if string(slst[0].ContextFile.Dig) != "3" {
		t.Errorf("slst[0]: %s, size: %d, depth: %d", string(slst[0].ContextFile.Dig), slst[0].ContextFile.Size, slst[0].Depth)
	}
	if string(slst[1].ContextFile.Dig) != "1" {
		t.Errorf("slst[1]: %s, size: %d, depth: %d", string(slst[1].ContextFile.Dig), slst[1].ContextFile.Size, slst[1].Depth)
	}
	if string(slst[2].ContextFile.Dig) != "14" {
		t.Errorf("slst[2]: %s, size: %d, depth: %d", string(slst[2].ContextFile.Dig), slst[2].ContextFile.Size, slst[2].Depth)
	}
